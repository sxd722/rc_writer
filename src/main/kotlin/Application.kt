        ...
        routing {
            post("/compile-rc") {
                ...
                val fullScript = buildString {
                    ...
                    appendLine("val Color.rc: Long get() = value.toLong()")
                    ...
                    appendLine("fun Modifier.background(color: Long, shape: RemoteRoundedCornerShape?): Modifier { var m: Modifier = this; if (shape != null) m = m.clip(RoundedRectShape(shape.r.value, shape.r.value, shape.r.value, shape.r.value)); return m.background(color) }")
                    appendLine("fun Modifier.border(width: RcDp, color: Long, shape: RemoteRoundedCornerShape?): Modifier = border(width.value, 0f, (color ushr 32).toInt(), if (shape != null) 2 else 0)")
                    appendLine()
                    ...
                    appendLine("fun RcScope.RemoteText(text: String, modifier: Modifier = Modifier, color: Any = 0L, fontSize: RcSp = 0.rsp, fontWeight: Any? = null) = Text(text = text, modifier = modifier, color = color, fontSize = fontSize, fontWeight = when(fontWeight) { is FontWeight -> fontWeight.weight.toFloat(); is Number -> fontWeight.toFloat(); else -> 0f })")
